package com.alinesno.infra.data.lake.adapter.service.impl;
...
import com.alinesno.infra.data.lake.adapter.dto.FileAttachmentDto;
import com.alinesno.infra.data.lake.adapter.service.CloudStorageConsumer;
import com.alinesno.infra.data.lake.adapter.service.FileDetailService;
...
public class CloudStorageServiceImpl implements CloudStorageConsumer {
    ...
    @Override
    public R<String> upload(File file , String filePath) {
        FileInfo info = fileStorageService
                .of(file)
                .setPath(filePath.endsWith("/")?filePath:filePath+"/")
                .setSaveFilename(IdUtil.getSnowflakeNextIdStr())
                .setProgressMonitor(new ProgressListener() {
                    @Override
                    public void start() {
                        log.debug("上传开始");
