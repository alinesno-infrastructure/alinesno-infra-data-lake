...
const emit = defineEmits(['update:showUploadModal', 'close', 'uploadSuccess'])
...
const startUpload = async () => {
  ...
  // if (successfulFiles.length < uploadFiles.value.length) {
  //   ElMessage.warning(`有 ${uploadFiles.value.length - successfulFiles.length} 个文件上传失败`)
  // }
...
const uploadSingleFile = async (index) => {
  ...
  try {
    ...
    // 调用上传API
    const response = await apiUploadFile(formData, config)
